...
const useStyles = makeStyles({
    button: {
        ...
        height: ({ size }) => size === 'large' ? '46px' : '36px',
        width: '100%',
        margin: '4px 0 4px',
...
const CustomButton = ({ className, color, size = 'large', children, onClick, isActive, ...props }) => {
    ...
    return (
        <Button
            className={`${className} ${classes.button} ${color === 'secondary' ? classes.buttonSecondary : ''}`}
            variant="contained"
            color="secondary"
            onClick={handleClick}
            {...props}
